...
def recent_stories_from(country):
    ...
    for details in content['responseData']['feed']['entries']:
        ...
        #} ) 
        entry = query_db('select * from entries where title = ?',[details['title']], one=True)
        if entry is None:
            db.execute('insert into entries (country, title, link, author, content) values (?,?,?,?,?)',
                [country, details['title'],details['link'],details['author'],h.unescape(details['contentSnippet'])])
            db.commit()
    entries = query_db('select * from entries where country = ? order by id', [country])
